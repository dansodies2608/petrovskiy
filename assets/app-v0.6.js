...
if (!firebase.apps.length) {
  ...
  firebase.app(); // Используем существующую инициализацию
}
...
async function requestNotificationPermission() {
  try {
    ...
    console.error('Ошибка в запросе разрешения:', error);
  }
}

// 4. Отправка токена на сервер
async function saveTokenToServer(token) {
  try {
    const response = await fetch('https://script.google.com/macros/s/AKfycbzmiq2x3zkNetsY9DPJym3tVSPkuC4YY8lWa7w270PILhW4XgaaLAjAb0AkHk-pr2GbVw/exec', {
      method: 'POST',
      headers: {
    ...
    console.log('Токен сохранен:', await response.text());
  } catch (error) {
    console.error('Ошибка сохранения токена:', error);
  }
}

// 5. Запускаем процесс при загрузке страницы
